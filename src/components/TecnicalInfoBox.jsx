...
const TechnicalInfoBox = ({ product }) => {
  // Intentamos obtener los detalles técnicos desde "fichaTecnica"
  let technicalDetails = null;
  if (product && product.fichaTecnica) {
    try {
      // Si es un JSON válido, lo convertimos a objeto
      technicalDetails = JSON.parse(product.fichaTecnica);
      if (typeof technicalDetails !== "object") {
        technicalDetails = { "Ficha Técnica": product.fichaTecnica };
      }
    } catch (error) {
      // Si falla el parseo, tratamos la ficha como texto plano
      technicalDetails = { "Ficha Técnica": product.fichaTecnica };
    }
  }
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
      <h2 className="text-xl font-semibold text-green-800 mb-6 border-b border-green-200 pb-2">
        Ficha Técnica
      </h2>
